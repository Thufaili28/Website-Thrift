...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	if ($update) {
		$sql = "UPDATE admin SET nama='$_POST[nama]', alamat='$_POST[alamat]', telepon='$_POST[telepon]', email='$_POST[email]', username='$_POST[username]', password='".md5($_POST["password"])."' WHERE id_admin='$_GET[key]'";
	} else {
		$sql = "INSERT INTO admin VALUES (NULL, '$_POST[nama]', '$_POST[alamat]', '$_POST[telepon]', '$_POST[email]', '$_POST[username]', '".md5($_POST["password"])."')";
	}
									...
									<tbody>
											...
											<?php if ($query = $connection->query("SELECT * FROM admin")): ?>
													...
													<tr>
															...
															<td>
																<div class="btn-group">
																	<a href="?page=pemilik&action=update&key=<?=$row['id_admin']?>" class="btn btn-warning btn-xs">Ubah</a>
																	<a href="?page=pemilik&action=delete&key=<?=$row['id_admin']?>" class="btn btn-danger btn-xs">Hapus</a>
																</div>
															</td>
													</tr>
													<?php endwhile ?>
